            ...
            <div class="row">
                <div class="col-md-3">
                    <div class="timer-container">
                        ...
                        <svg>
                            <circle class="progress-ring" id="progress-ring-1" r="90" cx="100" cy="100">
                            </circle>
                            <text class="timer-number" id="timer-number-1" x="100" y="110" text-anchor="middle"
                                alignment-baseline="middle">{{ $user['time'] }}</text>
                        </svg>
                    </div>
                </div>
                <div class="col-md-6">
                        ...
                        <div class="container" id="section">
                            ...
                            <form action="">
                                ...
                                <div class="quiz-options" data-correct-answer="{{ $user['correctAnswer'] }}">
                                    {{-- dd({{ $user['correctAnswer'] }}); --}}
                                    <input type="hidden" name="qid" value="{{ $user->id }}">

                                    <input type="radio" class="input-radio one-a " id="one-a" name="yes-1"
                                        value="a">
                                    ...
                                    <input type="radio" class="input-radio one-b " id="one-b" name="yes-1"
                                        value="b">
                                    <label class="radio-label " for="one-b">
                                        <span class="alphabet">B</span> {{ $user['option2'] }}
                                    </label>

                                    <input type="radio" class="input-radio one-c " id="one-c" name="yes-1"
                                        value="c">
                                    <label class="radio-label " for="one-c">
                                        <span class="alphabet">C</span>{{ $user['option3'] }}
                                    </label>

                                    <input type="radio" class="input-radio one-d " id="one-d" name="yes-1"
                                        value="d">
                                    <label class="radio-label " for="one-d">
                                        <span class="alphabet">D</span>{{ $user['option4'] }}</label>
                                </div>
                            </form>
                        </div>
    @endforeach
...
{{-- timer --}}
{{-- <script>
    var timeLimit = 20; // Total time in seconds
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        var timeLimit = 20; // Total time in seconds for each question
        var sections = document.querySelectorAll('section'); // Select all sections
        var currentSectionIndex = 0; // Keep track of the current section

        function startTimerForSection(sectionIndex) {
            var timeLeft = timeLimit; // Reset time for new section
            var section = sections[sectionIndex];

            // var timerDisplay = section.querySelector(
            //     '.timer-number'); // Select timer number in the current section
            // var circle = section.querySelector('.progress-ring'); // Select progress ring in the current section

            var timerDisplay = document.querySelector(
            `#timer-number-${sectionIndex + 1}`); // Adjusted for dynamic IDs
            var circle = document.querySelector(
            `#progress-ring-${sectionIndex + 1}`); // Adjusted for dynamic IDs
            ...
            var countdown = setInterval(function() {
                if (timeLeft >= 0) {
                    ...
                    if(timeLeft<0){
                        ...
                        var url="{{$users->hasMorePages()? $users->nextPageUrl():''}}";
        ...
        // Function to update the timer display and circle progress
        ...
        function goToNextSection() {
            // Get the selected answer for the current section

            var selectedAnswer = sections[currentSectionIndex].querySelector('input[name="yes-1' +
                currentSectionIndex + '"]:checked');

            //  var selectedAnswer = sections[currentSectionIndex].querySelector('input[name="yes-1' + currentSectionIndex + '"]:checked');


            // if (selectedAnswer) {
            //     // Check if the selected answer is correct
            //     if (selectedAnswer.value === "correct") {
            //         alert('Answer is correct! Moving to next question.');
            //     } else {
            //         alert("Wrong answer! Moving to next question.");
            //     }
            // } else {
            //     alert("No answer selected! Moving to next question.");
            // }
            ...
            //     // Show the next section and start its timer
            //     sections[currentSectionIndex].style.display = 'block';

            //     // Update the current section index in the URL hash
            //     window.location.href = `#section-${currentSectionIndex + 1}`;

            //     startTimerForSection(currentSectionIndex);
            // } else {
        ...
        sections.forEach((section, index) => {
            section.id = `section-${index + 1}`; // Set unique ids like section-1, section-2, etc.
        });
        startTimerForSection(0); // Start the timer for the first section
    });
</script>
